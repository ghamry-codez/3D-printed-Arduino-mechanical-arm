...
void moveMotor(int controlIn, int motorOut) {
    ...
    // Map the potentiometer position to pulse width
    pulse_wide = map(potVal, 800, 240, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH);

    // Invert the pulse width for the shoulder servo
    if (motorOut == shoulder) {
        pulse_wide = MAX_PULSE_WIDTH + MIN_PULSE_WIDTH - pulse_wide; // Invert the mapping
    }

    pulse_width = int(float(pulse_wide) / 1000000 * FREQUENCY * 4096); // Convert to PWM value
